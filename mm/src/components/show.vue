<template>
    <div id="big">
      ...
      <div id="nav2" style="color:black ;">
        <div id="search">
            <input type="text" id="searchInput" style="border-bottom-color:#000" @keyup.enter="sss">
        </div>
     ...
     <div class="clothes">
      <div class="ltitle">
        <h1 style="text-align:center;font-weight:400;line-height:30px">您已搜索</h1>
     <h1 v-text="zzkw"></h1>
      </div>
        <div class="lshaixuan">
        <ul class="lul1">
          <li>
           ...
           <ul class="lul2">
            ...
            <!-- <li v-text="$store.state.shopList.keywords"></li> -->
...
export default {
      data(){
        return{
          cc:100,
           msg:"颜色",
           msg1:"类别",
           zzkw:this.$store.state.shopList.keywords,
          items:[
            {
              id:0,
              title:"客户服务",        
          ...
          tiaozhuan:["xinpin","xifu","waitao","chenshan","zhenzhishan","poloshan","ku","lindai","xiezi","peishi","yanjing","youhui"],
        big:{
            color:["棕色","灰色","白色","紫色","红色","绿色","蓝色","黑色",'金色',"浅蓝色",'米色'],
           leibie:["Suits & Jackets","T 恤及POLO 衫 ","内衣","便鞋","口袋巾","围巾","外套","夹克",
              "毛衣","西服套装","正装鞋","牛仔","短袜","裤装","针织衫","领带","领带及口袋巾"]
          },
          have:[],
          cloth2:[],
          shai:[],
 ...
 methods:{
        ...
        {
         if(this.waitao[index].info===this.shopcar[i].info)
         {
        alert('已存在商品');
        ...
        if(i>=this.shopcar.length)
        {
          if(localStorage.getItem("currentUser")==null){
            alert("请先进行登录")
              ...
              this.shopcar.push({
            id:this.waitao[index].id,
            img:this.waitao[index].img,
            info:this.waitao[index].info,
            price:this.waitao[index].price,
            count:this.waitao[index].count,
...
else
{
  ...
  this.have=[];
  var arr=[];
  this.waitao=this.cloth2;
  console.log(this.waitao)
  ...
  this.waitao.forEach(function(item,index){
      var count = 0;
  ...
  this.waitao=this.have;
   ...
   this.waitao=this.have;
    ...
    mounted(){
    this.waitao=this.$store.state.shopList.sxShop;
    this.cloth2=this.waitao;
    } 
}
</script>
<style>
