<template>
    <div id="big">
     ...
     <h1>太阳镜</h1>
      ...
      <div class="lbottom">
          <ul>
              <li v-for="(item,index) in yanjing">
...
{
    if(this.yanjing[index].info===this.shopcar[i].info)
    {
        alert('已存在商品');
...
if(i>=this.shopcar.length)
{
   ...
   alert("请登录");
      ...
      this.shopcar.push({
      id:this.yanjing[index].id,
      img:this.yanjing[index].img,
      info:this.yanjing[index].info,
      price:this.yanjing[index].price,
      count:this.yanjing[index].count,
...
else
{
  ...
  this.yanjing.forEach(function(item,index){
      var count=0;
