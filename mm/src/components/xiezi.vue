...
{
    if(this.xiezi[index].info===this.shopcar[i].info)
    {
        alert('已存在商品');
...
if(i>=this.shopcar.length)
{
   ...
   alert("请登录");
      ...
      this.shopcar.push({
      id:this.xiezi[index].id,
      img:this.xiezi[index].img,
      info:this.xiezi[index].info,
      price:this.xiezi[index].price,
      count:this.xiezi[index].count,
...
else
{
  ...
  this.xiezi.forEach(function(item,index){
      var count=0;
