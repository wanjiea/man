...
export default {
   ...
   add(index){
    ...
    for(var i =0;i<this.shopcar.length;i++)
      {
          if(this.lindai[index].info===this.shopcar[i].info)
          {
              alert('已存在商品');
        ...
        if(i>=this.shopcar.length)
        {
          ...
          alert("请登录");
              ...
              this.shopcar.push({
              id:this.lindai[index].id,
              img:this.lindai[index].img,
              info:this.lindai[index].info,
              price:this.lindai[index].price,
              count:this.lindai[index].count,
  ...
  this.lindai.forEach(function(item,index){
      var count=0;
