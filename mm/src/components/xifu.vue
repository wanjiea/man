<template>
    <div id="big">
     ...
     <h1>西装套装及夹克</h1>
      ...
      <div class="lbottom">
          <ul>
              <li v-for="(item,index) in xizhuang">
...
{
    if(this.xizhuang[index].info===this.shopcar[i].info)
    {
        alert('已存在商品');
...
if(i>=this.shopcar.length)
{
   ...
   alert("请登录");
      ...
      this.shopcar.push({
     id:this.xizhuang[index].id,
      img:this.xizhuang[index].img,
      info:this.xizhuang[index].info,
      price:this.xizhuang[index].price,
      count:this.xizhuang[index].count,
...
else
{
  ...
  this.xizhuang.forEach(function(item,index){
      var count=0;
