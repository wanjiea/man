<template>
    <div id="big">
     ...
     <h1>衬衫</h1>
      ...
      <div class="lbottom">
          <ul>
              <li v-for="(item,index) in chenshan">
...
export default {
    ...
    for(var i =0;i<this.shopcar.length;i++)
      {
          if(this.chenshan[index].info===this.shopcar[i].info)
          {
              alert('已存在商品');
        ...
        if(i>=this.shopcar.length)
        {
          ...
          alert("请登录");
              ...
              this.shopcar.push({
              id:this.chenshan[index].id,
              img:this.chenshan[index].img,
              info:this.chenshan[index].info,
              price:this.chenshan[index].price,
              count:this.chenshan[index].count,
  ...
  this.chenshan.forEach(function(item,index){
      var count=0;
